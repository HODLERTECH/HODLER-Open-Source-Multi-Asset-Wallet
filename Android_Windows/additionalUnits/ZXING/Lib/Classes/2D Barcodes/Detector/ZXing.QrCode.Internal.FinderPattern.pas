...
type
  ...
  TFinderPatternHelpers = class(TResultPointHelpers)
    class function CreateFinderPattern(const posX, posY, estimatedModuleSize
      : Single; const count: integer): IFinderPattern; overload;
    class function CreateFinderPattern(const posX, posY, estimatedModuleSize
      : Single): IFinderPattern; overload;

    class procedure orderBestPatterns(const patterns
      : TArray<IFinderPattern>); static;
...
class function TFinderPatternHelpers.CreateFinderPattern(const posX, posY,
  estimatedModuleSize: Single; const count: integer): IFinderPattern;
begin
  result := ZXing.QrCode.Internal.FinderPatternImplementation.NewFinderPattern
    (posX, posY, estimatedModuleSize, count);
end;

class function TFinderPatternHelpers.CreateFinderPattern(const posX, posY,
  estimatedModuleSize: Single): IFinderPattern;
begin
  result := ZXing.QrCode.Internal.FinderPatternImplementation.NewFinderPattern
    (posX, posY, estimatedModuleSize);
end;

class procedure TFinderPatternHelpers.orderBestPatterns(const patterns
  : TArray<IFinderPattern>);
var
  zeroOneDistance, oneTwoDistance, zeroTwoDistance: Single;
  pointA, pointB, pointC, temp: IFinderPattern;
begin
