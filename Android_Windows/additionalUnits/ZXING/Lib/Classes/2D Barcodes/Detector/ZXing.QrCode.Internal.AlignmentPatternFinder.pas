...
type
  ...
  public
    ...
    /// </param>
    constructor Create(const image: TBitMatrix; const startX, startY, width,
      height: Integer; const moduleSize: Single;
      resultPointCallback: TResultPointCallback);
...
class function TAlignmentPatternFinder.centerFromEnd(const stateCount
  : TArray<Integer>; const pEnd: Integer): Single;
begin
...
function TAlignmentPatternFinder.foundPatternCross(const stateCount
...
function TAlignmentPatternFinder.handlePossibleCenter(const stateCount
  : TArray<Integer>; const i, j: Integer): IAlignmentPattern;
var
...
begin
  ...
  if (centerI <> -1) then
  begin
    estimatedModuleSize := (stateCount[0] + stateCount[1] + stateCount
      [2]) / 3.0;
    ...
    point := TAlignmentPatternHelpers.CreateAlignmentPattern(centerJ, centerI,
      estimatedModuleSize);
