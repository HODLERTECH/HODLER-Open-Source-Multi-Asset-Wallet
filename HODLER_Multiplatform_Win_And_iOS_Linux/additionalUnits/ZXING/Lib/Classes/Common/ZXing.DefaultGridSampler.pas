...
type
  ...
  public
    class function sampleGrid(const image: TBitMatrix;
      ...
      p3FromY, p4FromX, p4FromY: Single): TBitMatrix; overload; static;
...
class function TDefaultGridSampler.sampleGrid(const image: TBitMatrix;
  const dimensionX, dimensionY: Integer; const transform: TPerspectiveTransform)
  : TBitMatrix;
var
  bits: TBitMatrix;
      ...
      try
        ...
        begin
          bits[(TMathUtils.Asr(x, 1)), y] :=
            image[Floor(points[x]), Floor(points[x + 1])];
          Inc(x, 2);
        end
      except
