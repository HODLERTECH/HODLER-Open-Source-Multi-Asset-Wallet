...
type
  ...
  protected const
    ...
    class function foundPatternCross(const stateCount: TArray<Integer>)
      : Boolean; static;
  ...
  public
    ...
    function find(const hints: TDictionary<TDecodeHintType, TObject>)
...
function TFinderPatternFinder.find(const hints: TDictionary<TDecodeHintType,
  TObject>): TFinderPatternInfo;
var
...
begin
  ...
  pureBarcode := (hints <> nil) and
    (hints.ContainsKey(ZXing.DecodeHinttype.PURE_BARCODE));
  maxI := Self.image.Height;
  maxJ := Self.image.Width;
...
/// </returns>
class function TFinderPatternFinder.foundPatternCross(const stateCount
...
begin
  ...
  result := (Abs(moduleSize - (stateCount[0] shl INTEGER_MATH_SHIFT)) <
    maxVariance) and (Abs(moduleSize - (stateCount[1] shl INTEGER_MATH_SHIFT)) <
    maxVariance) and
    (Abs(3 * moduleSize - (stateCount[2] shl INTEGER_MATH_SHIFT)) <
    (3 * maxVariance)) and
...
begin
  ...
  if ((centerJ <> -1) and (Self.crossCheckDiagonal(Floor(centerI),
  ...
  begin
    ...
    if (not found) then
    begin
      point := TFinderPatternHelpers.CreateFinderPattern(centerJ, centerI,
        estimatedModuleSize);
      FPossibleCenters.Add(point);

      if Assigned(FresultPointCallback) then
...
begin
  ...
  begin
    ...
    i := 0;
    while (((i < FPossibleCenters.Count) and (FPossibleCenters.Count > 3))) do
    begin
      pattern := FPossibleCenters[i];
