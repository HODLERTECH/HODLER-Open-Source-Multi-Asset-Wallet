...
type
  ...
  public
    ...
    class function createBitMatrixParser(const BitMatrix: TBitMatrix)
...
class function TBitMatrixParser.createBitMatrixParser(const BitMatrix
  : TBitMatrix): TBitMatrixParser;
var
  dimension: Integer;
begin
  dimension := BitMatrix.Height;
  if ((dimension < 21) and ((dimension and $03) <> 1)) then
    Result := nil
  else
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if (resultOffset >= 0) and (resultOffset <= high(Result)) then
                // minchiata aggiunta da carlo
...
var
  formatInfoBits1, formatInfoBits2, dimension, i, j, jMin: Integer;
begin
  ...
  parsedFormatInfo := TFormatInformation.DecodeFormatInformation
...
begin
  ...
  if ((parsedVersion <> nil) and (parsedVersion.DimensionForVersion = dimension))
  then
  begin
...
begin
  ...
  begin
    dimension := BitMatrix.Height;
    TDataMask.forReference(parsedFormatInfo.DataMask).unmaskBitMatrix(BitMatrix,
      dimension);
  end;
end;
