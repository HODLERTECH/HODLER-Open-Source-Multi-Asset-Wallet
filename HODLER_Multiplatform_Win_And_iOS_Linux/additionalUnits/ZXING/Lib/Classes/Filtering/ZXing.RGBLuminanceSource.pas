...
begin
  ...
  begin
    ...
    begin
      ...
      b := P[x].rgbtBlue;
      luminances[offset + x] :=
        TMathUtils.Asr(3482 * r + 11721 * g + 1181 * b, 14);
    end;
  end;
end;
{$ELSE}
...
/// <param name="height">The height.</param>
/// <returns></returns>
function TRGBLuminanceSource.CreateLuminanceSource(const newLuminances
  : TArray<Byte>; const width, height: Integer): TLuminanceSource;
begin
  Result := TRGBLuminanceSource.Create(width, height);
  luminances := newLuminances;
end;

function TRGBLuminanceSource.DetermineBitmapFormat(const rgbRawBytes
  : TArray<Byte>; const width, height: Integer): TBitmapFormat;
var
  square, byteperpixel: Integer;
begin
  square := (width * height);
  ...
  case byteperpixel of
    1:
      Result := TBitmapFormat.Gray8;
    2:
      Result := TBitmapFormat.RGB565;
    3:
      Result := TBitmapFormat.RGB24;
    4:
      Result := TBitmapFormat.RGB32;
  else
    raise EArgumentException.Create
      ('The bitmap format could not be determined. Please specify the correct value.');
  end;
end;

procedure TRGBLuminanceSource.CalculateLuminance(const rgbRawBytes
  : TArray<Byte>; bitmapFormat: TBitmapFormat);
var
  len: Integer;
begin
  if (bitmapFormat = TBitmapFormat.Unknown) then
    bitmapFormat := DetermineBitmapFormat(rgbRawBytes, Self.width, Self.height);

  case bitmapFormat of
    TBitmapFormat.Gray8:
      begin
        if (Length(rgbRawBytes) < Length(luminances)) then
...
procedure TRGBLuminanceSource.CalculateLuminanceRGB565(const rgb565RawData
  : TArray<Byte>);
var
  luminanceIndex, index: Integer;
  byte1, byte2, b5, g5, r5, r8, g8, b8: Byte;
begin
  luminanceIndex := 0;
  index := 0;
  ...
  begin
    ...
    luminances[luminanceIndex] :=
      TMathUtils.Asr(RChannelWeight * r8 + GChannelWeight * g8 + BChannelWeight
      * b8, ChannelWeight);
    Inc(index, 2);
    Inc(luminanceIndex);
  end;
...
procedure TRGBLuminanceSource.CalculateLuminanceRGB24(const rgbRawBytes
  : TArray<Byte>);
var
  rgbIndex, luminanceIndex, r, g, b: Integer;
begin
  rgbIndex := 0;
  luminanceIndex := 0;
  ...
  begin
    ...
    Inc(rgbIndex);
    luminances[luminanceIndex] :=
      TMathUtils.Asr(RChannelWeight * r + GChannelWeight * g + BChannelWeight *
      b, ChannelWeight);
    Inc(luminanceIndex)
  end;
end;

procedure TRGBLuminanceSource.CalculateLuminanceBGR24(const rgbRawBytes
  : TArray<Byte>);
var
  rgbIndex, luminanceIndex, b, g, r: Byte;
begin
  rgbIndex := 0;
  luminanceIndex := 0;
  while (((rgbIndex < Length(rgbRawBytes)) and
...
procedure TRGBLuminanceSource.CalculateLuminanceRGB32(const rgbRawBytes
  : TArray<Byte>);
var
  rgbIndex, luminanceIndex, r, g, b: Byte;
begin
  rgbIndex := 0;
  luminanceIndex := 0;
  while (((rgbIndex < Length(rgbRawBytes)) and
...
procedure TRGBLuminanceSource.CalculateLuminanceBGR32(const rgbRawBytes
  : TArray<Byte>);
var
  rgbIndex, luminanceIndex, b, g, r: Byte;
begin
...
procedure TRGBLuminanceSource.CalculateLuminanceBGRA32(const rgbRawBytes
  : TArray<Byte>);
var
  rgbIndex, luminanceIndex, b, g, r, alpha: Byte;
  luminance: Byte;
begin
  rgbIndex := 0;
  luminanceIndex := 0;
  ...
  begin
    ...
    luminances[luminanceIndex] :=
      (TMathUtils.Asr(luminance * alpha, 8) +
      TMathUtils.Asr(255 * (255 - alpha), 8));
    Inc(luminanceIndex);
  end;
end;

procedure TRGBLuminanceSource.CalculateLuminanceRGBA32(const rgbRawBytes
  : TArray<Byte>);
var
  rgbIndex, luminanceIndex, r, g, b, alpha: Byte;
  luminance: Byte;
begin
  rgbIndex := 0;
  luminanceIndex := 0;
  ...
  begin
    ...
    luminance := TMathUtils.Asr(RChannelWeight * r + GChannelWeight * g +
      BChannelWeight * b, ChannelWeight);
    luminances[luminanceIndex] := TMathUtils.Asr(luminance * alpha, 8) +
      TMathUtils.Asr(255 * (255 - alpha), 8);
    Inc(luminanceIndex);
  end;
end;

procedure TRGBLuminanceSource.CalculateLuminanceARGB32(const rgbRawBytes
  : TArray<Byte>);
var
  rgbIndex, luminanceIndex, r, g, b, alpha: Byte;
  luminance: Byte;
