...
// -------------------------------------------------------------------------------- //
  ...
  private
    ...
    procedure SetRedirectsWithGET(const Value: THTTPRedirectsWithGET);
...
var
  FHTTPClientExtDict: TDictionary<THTTPClient, THTTPClientExt> = nil;
...
function THTTPClient.BeginMerge(const AURL: string; const ASource: TStream; const AHeaders: TNetHeaders): IAsyncResult;
begin
...
function GetEncodingMIMEName(enc:TEncoding):string;
begin
  result:='urf-8';
end;
...
begin
  ...
  try
    LHeaders := [TNetHeader.Create(sContentType, 'application/x-www-form-urlencoded; charset=' + GetEncodingMIMEName(LEncoding))] + AHeaders;  // do not translate
...
begin
  ...
  Result := DoExecuteAsync(nil, nil, sHTTPMethodPost, TURI.Create(AURL),
    TStringStream.Create(LParams, LEncoding, False), AResponseContent, LHeaders, True);
end;

function THTTPClient.BeginPost(const AsyncCallback: TAsyncCallback; const AURL: string; const ASource: TStrings;
  const AResponseContent: TStream; const AEncoding: TEncoding; const AHeaders: TNetHeaders): IAsyncResult;
var
  LParams: string;
  LHeaders: TNetHeaders;
  LEncoding: TEncoding;
...
begin
  Result := DoExecuteAsync(nil, nil, sHTTPMethodPost, TURI.Create(AURL),
    TFileStream.Create(ASourceFile, fmOpenRead or fmShareDenyWrite), AResponseContent, AHeaders, True);
end;
...
var
  LExt: THTTPClientExt;
begin
  LExt.FRedirectsWithGET := CHTTPDefRedirectsWithGET;
  LExt.FSecureProtocols := CHTTPDefSecureProtocols;
  THTTPClientExt(AExt) := LExt;
  if FHTTPClientExtDict = nil then
    Exit;
  TMonitor.Enter(FHTTPClientExtDict);
  try
    if FHTTPClientExtDict.TryGetValue(Self, LExt) then
      THTTPClientExt(AExt) := LExt;
  finally
    TMonitor.Exit(FHTTPClientExtDict);
  end;
end;
...
begin
  ...
  try
{$IFDEF AUTOREFCOUNT}
    LRelease := not FHTTPClientExtDict.ContainsKey(Self);
{$ENDIF}
    LExt := THTTPClientExt(Value);
    FHTTPClientExtDict.AddOrSetValue(Self, LExt);
{$IFDEF AUTOREFCOUNT}
...
begin
  ...
  TMonitor.Enter(FHTTPClientExtDict);
  try
    FHTTPClientExtDict.Remove(Self);
  finally
    TMonitor.Exit(FHTTPClientExtDict);
  end;
end;

function THTTPClientHelper.GetRedirectsWithGET: THTTPRedirectsWithGET;
var
  LExt: THTTPClientExt;
begin
  GetExt(LExt);
  Result := LExt.FRedirectsWithGET;
end;

function THTTPClientHelper.GetSecureProtocols: THTTPSecureProtocols;
var
  LExt: THTTPClientExt;
begin
  GetExt(LExt);
  Result := LExt.FSecureProtocols;
end;

procedure THTTPClientHelper.SetRedirectsWithGET(const Value: THTTPRedirectsWithGET);
var
  LExt: THTTPClientExt;
begin
  GetExt(LExt);
  LExt.FRedirectsWithGET := Value;
  SetExt(LExt);
end;

procedure THTTPClientHelper.SetSecureProtocols(const Value: THTTPSecureProtocols);
var
  LExt: THTTPClientExt;
begin
  GetExt(LExt);
  LExt.FSecureProtocols := Value;
  SetExt(LExt);
end;

class function THTTPClientHelper.IsRedirect(const AStatusCode: Integer): Boolean;
begin
  Result := False;
  case AStatusCode of
...
function THTTPClientHelper.IsAutoRedirect(const AResponse: THTTPResponse): Boolean;
begin
  Result := HandleRedirects and IsRedirect(AResponse.GetStatusCode) and
    (AResponse.GetHeaderValue(sLocation).Trim <> '');
end;

function THTTPClientHelper.IsAutoRedirectWithGET(const ARequest: THTTPRequest; const AResponse: THTTPResponse): Boolean;
var
...
function THTTPClientHelper.ComposeRedirectURL(const ARequest: THTTPRequest;
...
initialization
  FHTTPClientExtDict := TDictionary<THTTPClient, THTTPClientExt>.Create;

finalization
  FreeAndNil(FHTTPClientExtDict);
