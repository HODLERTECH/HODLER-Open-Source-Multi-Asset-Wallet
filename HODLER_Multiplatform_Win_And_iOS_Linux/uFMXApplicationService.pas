    ...
    case TempMsg.Message of
      ...
      WM_MOVING :
        if not(Application.Terminated) and
           ...
           (Application.MainForm is TfrmHome ) then
          TfrmHome(Application.MainForm).NotifyMoveOrResizeStarted;
      ...
      CEF_AFTERCREATED :
        if not(Application.Terminated) and
           (Application.MainForm <> nil) and
           (Application.MainForm is TfrmHome ) then
          TfrmHome (Application.MainForm).DoBrowserCreated;

      CEF_DESTROY :
        if not(Application.Terminated) and
           (Application.MainForm <> nil) and
           (Application.MainForm is TfrmHome) then
          TfrmHome(Application.MainForm).DoDestroyParent;
    end;
  {$ENDIF}
