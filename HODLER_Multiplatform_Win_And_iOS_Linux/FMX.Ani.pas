...
uses
  System.Classes, System.UITypes, System.Rtti, System.Generics.Collections,
  FMX.Types, FMX.Graphics, DW.ThreadedTimer;

type

  ITriggerAnimation = interface
    ['{8A291102-742F-45CB-9159-4E1D283AAF20}']
    procedure StartTriggerAnimation(const AInstance: TFmxObject;
      const ATrigger: string);
    procedure StartTriggerAnimationWait(const AInstance: TFmxObject;
      const ATrigger: string);
  end;
  ...
  private
    class var FDestroyer: TAnimationDestroyer;
  private
    class procedure CreateDestroyer;
    class procedure Uninitialize;
  public
    { animations }
    class procedure StartAnimation(const Target: TFmxObject;
      const AName: string);
    class procedure StopAnimation(const Target: TFmxObject;
      const AName: string);
    class procedure StartTriggerAnimation(const Target: TFmxObject;
      const AInstance: TFmxObject; const ATrigger: string);
    class procedure StartTriggerAnimationWait(const Target: TFmxObject;
      const AInstance: TFmxObject; const ATrigger: string);
    class procedure StopTriggerAnimation(const Target: TFmxObject;
      const AInstance: TFmxObject; const ATrigger: string);
    { default }
    class procedure DefaultStartTriggerAnimation(const Target,
      AInstance: TFmxObject; const ATrigger: string); static;
    class procedure DefaultStartTriggerAnimationWait(const Target: TFmxObject;
      const AInstance: TFmxObject; const ATrigger: string);
    { animation property }
    ...
    class procedure AnimateFloatDelay(const Target: TFmxObject;
      const APropertyName: string; const NewValue: Single;
      Duration: Single = 0.2; Delay: Single = 0.0;
      AType: TAnimationType = TAnimationType.In;
      AInterpolation: TInterpolationType = TInterpolationType.Linear);
    class procedure AnimateFloatWait(const Target: TFmxObject;
      const APropertyName: string; const NewValue: Single;
      Duration: Single = 0.2; AType: TAnimationType = TAnimationType.In;
      AInterpolation: TInterpolationType = TInterpolationType.Linear);
    class procedure AnimateInt(const Target: TFmxObject;
      ...
      Duration: Single = 0.2; AType: TAnimationType = TAnimationType.In;
      AInterpolation: TInterpolationType = TInterpolationType.Linear);
    class procedure AnimateIntWait(const Target: TFmxObject;
      const APropertyName: string; const NewValue: Integer;
      Duration: Single = 0.2; AType: TAnimationType = TAnimationType.In;
      AInterpolation: TInterpolationType = TInterpolationType.Linear);
    class procedure AnimateColor(const Target: TFmxObject;
      ...
      Duration: Single = 0.2; AType: TAnimationType = TAnimationType.In;
      AInterpolation: TInterpolationType = TInterpolationType.Linear);
    class procedure StopPropertyAnimation(const Target: TFmxObject;
  ...
  public
    class var AniFrameRate: Integer;
  private type
    TTriggerRec = record
      Name: string;
      Prop: TRttiProperty;
  ...
  private
    ...
    procedure ParseTriggers(const AInstance: TFmxObject;
      Normal, Inverse: Boolean);
    class procedure Uninitialize;
  protected
  ...
  public
    ...
    property AnimationType: TAnimationType read FAnimationType
      write FAnimationType default TAnimationType.In;
    ...
    property Interpolation: TInterpolationType read FInterpolation
      write FInterpolation default TInterpolationType.Linear;
    ...
    property TriggerInverse: TTrigger read FTriggerInverse
      write SetTriggerInverse;
  ...
  private type
    ...
    private
      [Weak]
      FAnimation: TBitmapListAnimation;
    protected
  ...
  public
    ...
    destructor Destroy; override;
  published
    property AnimationBitmap: TBitmap read FAnimationBitmap
      write SetAnimationBitmap;
    property AnimationLookup: string read FAnimationLookup
      write SetAnimationLookup;
    property AnimationCount: Integer read FAnimationCount write FAnimationCount;
    property AnimationRowCount: Integer read FAnimationRowCount
      write SetAnimationRowCount default 1;
    property AnimationType default TAnimationType.In;
    property AutoReverse default False;
    property Enabled default False;
    property Delay;
...
function InterpolateElastic(t, B, C, D, A, P: Single;
  AType: TAnimationType): Single;
...
uses
  System.Types, System.Math, System.SysUtils, System.StrUtils, {$IFDEF MACOS}Macapi.CoreFoundation, {$ENDIF}
  ...
  case AType of
    TAnimationType.In:
      begin
        ...
        t := t - 1;
        Result := -(A * Power(2, (10 * t)) * Sin((t * D - S) * (2 * Pi)
          / P)) + B;
      end;
    TAnimationType.Out:
      begin
    ...
    TAnimationType.InOut:
      begin
        ...
        begin
          t := t - 1;
          Result := -0.5 * (A * Power(2, (10 * t)) * Sin((t * D - S) * (2 * Pi)
            / P)) + B;
        end
        else
        begin
          t := t - 1;
          Result := A * Power(2, (-10 * t)) * Sin((t * D - S) * (2 * Pi) / P) *
            0.5 + C + B;
        end;
      end;
  else
...
class procedure TAnimator.StartAnimation(const Target: TFmxObject;
const AName: string);
var
  I: Integer;
  E: TAnimation;
...
class procedure TAnimator.StopAnimation(const Target: TFmxObject;
const AName: string);
var
  I: Integer;
  E: TAnimation;
...
class procedure TAnimator.StartTriggerAnimation(const Target: TFmxObject;
const AInstance: TFmxObject; const ATrigger: string);
var
...
class procedure TAnimator.DefaultStartTriggerAnimation(const Target: TFmxObject;
const AInstance: TFmxObject; const ATrigger: string);
var
  I: Integer;
  Control: IControl;
begin
  if (Target <> nil) and (Target.Children <> nil) then
    ...
    begin
      ...
      if Supports(Target.Children[I], IControl, Control) and Control.Locked and
        not Control.HitTest then
        StartTriggerAnimation(Target.Children[I], AInstance, ATrigger);
    end;
end;

class procedure TAnimator.StartTriggerAnimationWait(const Target: TFmxObject;
const AInstance: TFmxObject; const ATrigger: string);
var
  Animatable: ITriggerAnimation;
begin
...
class procedure TAnimator.DefaultStartTriggerAnimationWait(const Target,
  AInstance: TFmxObject; const ATrigger: string);
var
  I: Integer;
  Control: IControl;
begin
...
class procedure TAnimator.StopTriggerAnimation(const Target: TFmxObject;
const AInstance: TFmxObject; const ATrigger: string);
var
  Item: TFmxObject;
  Control: IControl;
begin
  if Target.Children <> nil then
    ...
    begin
      ...
      if Supports(Item, IControl, Control) and Control.Locked and not Control.HitTest
      then
...
class procedure TAnimator.AnimateColor(const Target: TFmxObject;
const APropertyName: string; NewValue: TAlphaColor; Duration: Single = 0.2;
AType: TAnimationType = TAnimationType.In;
AInterpolation: TInterpolationType = TInterpolationType.Linear);
var
...
class procedure TAnimator.AnimateFloat(const Target: TFmxObject;
const APropertyName: string; const NewValue: Single; Duration: Single = 0.2;
AType: TAnimationType = TAnimationType.In;
AInterpolation: TInterpolationType = TInterpolationType.Linear);
var
...
class procedure TAnimator.AnimateFloatDelay(const Target: TFmxObject;
const APropertyName: string; const NewValue: Single; Duration: Single = 0.2;
...
class procedure TAnimator.AnimateFloatWait(const Target: TFmxObject;
const APropertyName: string; const NewValue: Single; Duration: Single = 0.2;
AType: TAnimationType = TAnimationType.In;
AInterpolation: TInterpolationType = TInterpolationType.Linear);
var
...
class procedure TAnimator.AnimateInt(const Target: TFmxObject;
const APropertyName: string; const NewValue: Integer; Duration: Single = 0.2;
AType: TAnimationType = TAnimationType.In;
AInterpolation: TInterpolationType = TInterpolationType.Linear);
var
...
class procedure TAnimator.AnimateIntWait(const Target: TFmxObject;
const APropertyName: string; const NewValue: Integer; Duration: Single = 0.2;
AType: TAnimationType = TAnimationType.In;
AInterpolation: TInterpolationType = TInterpolationType.Linear);
var
...
class procedure TAnimator.StopPropertyAnimation(const Target: TFmxObject;
const APropertyName: string);
var
  I: Integer;
begin
  ...
  begin
    if (Target.Children[I] is TCustomPropertyAnimation) and
      (CompareText(TCustomPropertyAnimation(Target.Children[I]).PropertyName,
      APropertyName) = 0) then
  ...
  begin
    case FInterpolation of
      ...
      TInterpolationType.Elastic:
        Result := InterpolateElastic(FTime, 0, 1, FDuration, 0, 0,
          FAnimationType);
...
begin
  ...
  begin
    ...
    else if FAutoReverse and (FTickCount = 0) then
    begin
      Inc(FTickCount);
      FInverse := True;
      FTime := FDuration;
...
begin
  ...
  if (Abs(FDuration) < 0.001) or (Root = nil) or (csDesigning in ComponentState)
  then
  begin
    { immediate animation }
...
procedure TAnimation.ParseTriggers(const AInstance: TFmxObject;
Normal, Inverse: Boolean);
var
  t: TRttiType;
  P: TRttiProperty;
...
begin
  ...
  if ((Inverse and (FInverseTriggerList <> nil)) or not Inverse) and
    ((Normal and (FTriggerList <> nil)) or not Normal) then
    Exit;
...
procedure TAnimation.StartTrigger(const AInstance: TFmxObject;
const ATrigger: string);
var
  V: TValue;
  StartValue: Boolean;
...
begin
  ...
  begin
    if (FInverseTriggerList <> nil) and (FInverseTriggerList.Count > 0) and ContainsInTriggerInverse
    then
    begin
      StartValue := False;
    ...
    if (FTriggerList <> nil) and (FTriggerList.Count > 0) and ContainsInTrigger
    then
    begin
      StartValue := False;
...
procedure TAnimation.StopTrigger(const AInstance: TFmxObject;
const ATrigger: string);
begin
  if AInstance = nil then
    Exit;
  if (FTriggerInverse <> '') and string(FTriggerInverse)
    .ToLower.Contains(ATrigger.ToLower) then
    Stop;
  if (FTrigger <> '') and string(FTrigger).ToLower.Contains(ATrigger.ToLower)
  then
    Stop;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if not ClonePropertiesCache.TryGetValue(FInstance.ClassName, Properties)
        then
        begin
...
begin
  ...
  begin
    ...
    begin
      P := t.GetProperty(FPath);
      if (P <> nil) and (P.PropertyType.TypeKind = tkFloat) then
        P.SetValue(FInstance, InterpolateSingle(FStartFloat, FStopFloat,
          NormalizedTime));
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      P := t.GetProperty(FPath);
      if (P <> nil) and (P.PropertyType.TypeKind in [tkInteger, tkFloat]) then
        P.SetValue(FInstance, Round(InterpolateSingle(FStartValue, FStopValue,
          NormalizedTime)));
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      P := t.GetProperty(FPath);
      if (P <> nil) and (P.PropertyType.IsOrdinal) then
        P.SetValue(FInstance, InterpolateColor(FStartColor, FStopColor,
          NormalizedTime));
    end;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if (I < FStopGradient.Points.Count) and
            (I < FStartGradient.Points.Count) then
            TargetGradientTmp.Points[I].Color :=
              FMX.Utils.InterpolateColor(FStartGradient.Points[I].Color,
              FStopGradient.Points[I].Color, NormalizedTime);
        end;
            ...
            if Bitmap.Canvas.BeginScene then
              try
                ...
                Bitmap.Canvas.DrawBitmap(FStopBitmap,
                  ...
                  RectF(0, 0, FStopBitmap.Width / Bitmap.BitmapScale,
                  FStopBitmap.Height / Bitmap.BitmapScale), NormalizedTime);
                Bitmap.Canvas.DrawBitmap(FStartBitmap,
                  RectF(0, 0, Bitmap.Width, Bitmap.Height),
                  RectF(0, 0, FStartBitmap.Width / Bitmap.BitmapScale,
                  FStartBitmap.Height / Bitmap.BitmapScale),
                  1 - NormalizedTime);
              finally
...
procedure TBitmapListAnimation.TAnimationBitmap.ReadStyleLookup
  (Reader: TReader);
begin
...
begin
  ...
  begin
    ...
    if Supports(ResourceObject, IMultiResBitmapObject, MultiResObject) and
      (WorkBitmap <> nil) then
    begin
      Item := MultiResObject.MultiResBitmap.ItemByScale(WorkBitmap.BitmapScale,
        False, False);
      if Item <> nil then
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if (FAnimationBitmap.BitmapScale <> WorkBitmap.BitmapScale) and
            (FAnimationLookup <> '') then
        ...
        begin
          ...
          WorkBitmap.SetSize(SourceBitmap.Width div AnimationColCount,
            SourceBitmap.Height div FAnimationRowCount);

          CurrentIndex := Trunc(NowValue);
          if CurrentIndex > FAnimationCount - 1 then
            CurrentIndex := FAnimationCount - 1;

          LeftPos := (CurrentIndex mod AnimationColCount) *
            (SourceBitmap.Width div AnimationColCount);
          TopPos := (CurrentIndex div AnimationColCount) *
            (SourceBitmap.Height div AnimationRowCount);

          if WorkBitmap.Canvas.BeginScene then
            try
              WorkBitmap.Canvas.Clear(0);
              WorkBitmap.Canvas.DrawBitmap(SourceBitmap,
                TRectF.Create(LeftPos, TopPos, LeftPos + WorkBitmap.Width,
                TopPos + WorkBitmap.Height),
                TRectF.Create(0, 0, WorkBitmap.Width / WorkBitmap.BitmapScale,
  ...
  begin
    ...
    begin
      ...
      begin
        P := t.GetProperty(FPath);
        if (P <> nil) and P.PropertyType.IsOrdinal then
          P.SetValue(FInstance, InterpolateColor(TColorKey(Key1).Value,
  ...
  begin
    ...
    begin
      ...
      begin
        P := t.GetProperty(FPath);
        if (P <> nil) and (P.PropertyType.TypeKind = tkFloat) then
          P.SetValue(FInstance, InterpolateSingle(TFloatKey(Key1).Value,
            TFloatKey(Key2).Value, (NormalizedTime - TFloatKey(Key1).Key) /
            (TFloatKey(Key2).Key - TFloatKey(Key1).Key)))
        else if (P <> nil) and P.PropertyType.IsOrdinal then
          P.SetValue(FInstance, Round(InterpolateSingle(TFloatKey(Key1).Value,
            TFloatKey(Key2).Value, (NormalizedTime - TFloatKey(Key1).Key) /
            (TFloatKey(Key2).Key - TFloatKey(Key1).Key))));
      end;
    end;
...
RegisterFmxClasses([TColorAnimation, TGradientAnimation, TFloatAnimation,
  TIntAnimation, TRectAnimation, TBitmapAnimation, TBitmapListAnimation,
  TColorKeyAnimation, TFloatKeyAnimation]);
