...
begin
  CHECK_DIGIT_ENCODINGS := TArray<Integer>.Create($18, $14, $12, $11, $0C, $06,
    $03, $0A, $09, $05);
...
class function TUPCEANExtension5Support.decodeRow(const rowNumber: Integer;
  const row: IBitArray; const extensionStartRange: TArray<Integer>)
  : TReadResult;
var
  res: TStringBuilder;
  ending: Integer;
...
begin
  ...
  resultPoints := TArray<IResultPoint>.Create
    (TResultPointHelpers.CreateResultPoint((extensionStartRange[0] +
    extensionStartRange[1]) div 2, rowNumber),
    TResultPointHelpers.CreateResultPoint(ending, rowNumber));

  extensionResult := TReadResult.Create(resultString, nil, resultPoints,
    TBarcodeFormat.UPC_EAN_EXTENSION);
...
var
  ...
  rowOffset, x, lgPatternFound, checkDigit: Integer;
begin
  Result := -1;
...
class function TUPCEANExtension5Support.determineCheckDigit(const lgPatternFound
  : Integer; var checkDigit: Integer): Boolean;
var
  i: Integer;
begin
  Result := false;
...
class function TUPCEANExtension5Support.parseExtensionString(const raw: String)
  : TResultMetadata;
var
  value: String;
...
begin
  ...
  dictionary1[ZXing.ResultMetadataType.SUGGESTED_PRICE] :=
    TResultMetadata.CreateStringMetadata(value);
...
var
  currency, unitsString, hundredthsString: string;
  rawAmount, hundredths: Integer;
begin
  Result := '';
