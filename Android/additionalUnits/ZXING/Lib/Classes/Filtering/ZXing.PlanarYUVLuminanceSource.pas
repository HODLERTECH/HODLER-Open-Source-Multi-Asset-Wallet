...
type
  ...
  private
    yuvData: TArray<Byte>;
    dataWidth, dataHeight, left, top: Integer;
  ...
  public
    ...
    function getRow(const y: Integer; row: TArray<Byte>): TArray<Byte>;
      override;
    function crop(const left, top, width, height: Integer)
...
begin
  ...
  if ((left + width > dataWidth) or (top + height > dataHeight)) then
...
/// </returns>
function TPlanarYUVLuminanceSource.getRow(const y: Integer; row: TArray<Byte>)
  : TArray<Byte>;
var
  width, offset: Integer;
begin
  if ((y < 0) or (y >= height)) then
    raise EArgumentException.Create('Requested row is outside the image: ' +
      IntToStr(y));
...
var
  width, height, area, y: Integer;
  inputOffset, outputOffset: Integer;
  Matrix, yuv: TArray<Byte>;
begin
  width := Self.width;
  height := Self.height;
...
/// </returns>
function TPlanarYUVLuminanceSource.crop(const left, top, width, height: Integer)
  : TLuminanceSource;
begin
  Result := TPlanarYUVLuminanceSource.Create(yuvData, dataWidth, dataHeight,
    Self.left + left, Self.top + top, width, height, false);
end;
...
var
  ...
  yuv: TArray<Byte>;
  x, y, inputOffset, outputOffset: Integer;
  grey: Byte;
begin
...
procedure TPlanarYUVLuminanceSource.reverseHorizontal(const width,
  height: Integer);
var
  yuvData: TArray<Byte>;
  y, x1, x2, middle, rowStart: Integer;
  temp: Byte;
begin
...
function TPlanarYUVLuminanceSource.CreateLuminanceSource(const newLuminances
